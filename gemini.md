...
- Redesigned the Notch progress indicator to only show the loading spinner during active progress (hiding "n in progress" text) and keep the width tight to only hold the spinner (60pt left, 60pt right). Upon completion, it expands to show a green checkmark with "n progress done" (130pt left, 130pt right) for 2 seconds before collapsing.
